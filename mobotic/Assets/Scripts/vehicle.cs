...
public class Vehicle : MonoBehaviour
{
    ...
    public wheel[] wheels;
    public GameObject wheelPrefabState0; // Wheel prefab for wheelState 0
    public GameObject wheelPrefabState1; // Wheel prefab for wheelState 1
    public GameObject wheelPrefabState2; // Wheel prefab for wheelState 2
    ...
    private void Start()
    {
        ...
        {
            GameObject selectedPrefab = GetWheelPrefab(wheels[i].wheelState);
            if (selectedPrefab == null)
            {
                Debug.LogError($"Wheel prefab for state {wheels[i].wheelState} not assigned in Vehicle.");
                return;
            }

            // Instantiate and configure each wheel
            wheelObjects[i] = Instantiate(selectedPrefab, transform);
            ...
            // Flip the wheel graphic if flipWheel is true
            Transform wheelVisual = wheelObjects[i].transform;
            if (wheels[i].flipWheel)
            {
                Vector3 localScale = wheelVisual.localScale;
                localScale.x = Mathf.Abs(localScale.x) * -1; // Ensure the scale is flipped over the x-axis
                wheelVisual.localScale = localScale;
            }
    ...
    private GameObject GetWheelPrefab(int wheelState)
    {
        // Return the appropriate wheel prefab based on the wheelState
        switch (wheelState)
        {
            case 0:
                return wheelPrefabState0;
            case 1:
                return wheelPrefabState1;
            case 2:
                return wheelPrefabState2;
            default:
                Debug.LogWarning($"Invalid wheelState {wheelState}. Defaulting to state 0 prefab.");
                return wheelPrefabState0;
        }
    }
    ...
    {
        // Calculate rotation based on velocity along the wheel's rolling axis
        Vector3 localVelocity = wheelObj.transform.InverseTransformDirection(rb.GetPointVelocity(wheel.wheelWorldPosition));
        float wheelRotationSpeed = localVelocity.z * 360 / wheel.wheelCircumference * (wheel.flipWheel ? -1 : 1); // Using local Z-velocity for rotation
        wheelVisual.Rotate(Vector3.right, wheelRotationSpeed * Time.fixedDeltaTime, Space.Self); // Roll along local X-axis
